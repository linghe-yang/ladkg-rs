...
for x in num_proc:
	...
	cross_p = [[x0,y0,z0] for x0 in epsilon for y0 in delta for z0 in tri]
	iterations = 1
	latencies = []
	command_template = 'ln -sf node runnode '
	run_command = 'nohup ./runnode --config nodes-{ind}.json --ip ip_file --sleep 100 --epsilon {ep} --delta {del} --val {val} --tri {tri} --vsstype hyb --syncer syncer --batch 100 --rand 10000 --expo 2 > /dev/null '
	for arr in cross_p:
		latency_arr = []
		...
		for iterate in range(iterations):
			...
			epsilon = arr[0]
			delta = arr[1]
			values_arr = np.linspace(start_val-arr[2]/2,start_val+arr[2]/2,num_processes)
	...
	print(latencies)
	import csv
	with open(f"latencies_n_{x}_abraham.txt","a") as f:
